@php $items = [
    ['label'=>'Home','icon'=>'home','route'=>'ecomx-anyniche.home'],
    ['label'=>'Shop','icon'=>'grid','route'=>'ecomx-anyniche.shop'],
    ...
    @foreach($items as $n)
        @if(isset($n['action']))
            <button type="button" class="bottom-nav__item {{ $ab === strtolower($n['label']) ? 'is-active' : '' }}" style="position:relative" @click="$store.ui.{{ $n['action'] }}=true">
                <span class="bottom-nav__icon"><x-anyniche::icon :name="$n['icon']" /></span>
                <span>{{ $n['label'] }}</span>
