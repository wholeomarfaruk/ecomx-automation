    ...
    <template x-for="col in ({{ json_encode($cols) }})[mega]" :key="col[0]">
        <div style="display:flex;flex-direction:column;gap:14px">
            ...
            <template x-for="link in col[1]" :key="link">
                <a :href="'{{ route('ecomx-anyniche.category') }}'" x-text="link" style="font-size:13.5px;color:rgba(var(--pri-rgb),.72)"></a>
            </template>
        </div>
    </template>
    <a href="{{ route('ecomx-anyniche.category') }}" style="position:relative;border-radius:14px;overflow:hidden;aspect-ratio:4/3">
        <img src="{{ config('ecomx-anyniche.unsplash') }}photo-1445205170230-053b83016050?q=80&w=600&auto=format&fit=crop" alt="The Autumn Edit" style="width:100%;height:100%;object-fit:cover">
