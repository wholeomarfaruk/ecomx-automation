<div x-data x-init="$store.pageName = { name: 'Edit Coupon', slug: 'sales-coupons-edit' }">
    ...
    <div class="flex flex-wrap items-center justify-between gap-4 mb-6">
        <div class="flex items-center gap-3">
            ...
            <h1 class="text-lg font-semibold text-gray-900">{{ $code ?: 'Edit Coupon' }}</h1>
        </div>
    ...
    <div class="grid grid-cols-12 gap-6">
        ...
        <div class="col-span-12 lg:col-span-8 space-y-6">
            ...
            {{-- Recent Usages --}}
            <div class="bg-white rounded-2xl shadow-sm border border-gray-200 p-6">
                <h2 class="text-sm font-semibold text-gray-800 mb-4">Recent Usages</h2>
                <div class="space-y-2">
                    @forelse($usages as $usage)
                        <div class="flex items-center justify-between rounded-lg border border-gray-200 px-4 py-2.5">
                            <div>
                                <a href="{{ route('admin.sales.orders.show', $usage->order_id) }}" wire:navigate class="text-sm text-indigo-600 hover:underline">Order #{{ $usage->order_id }}</a>
                                <span class="block text-xs text-gray-400">{{ $usage->customer?->full_name ?? 'Guest' }} · {{ $usage->used_at->format('d M, Y H:i') }}</span>
                            </div>
                            <span class="text-sm font-medium text-gray-800">{{ number_format($usage->discount_amount, 2) }}</span>
                        </div>
                    @empty
                        <p class="text-sm text-gray-400">This coupon hasn't been used yet.</p>
                    @endforelse
                </div>
            </div>
        </div>
