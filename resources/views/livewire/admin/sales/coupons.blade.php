<div x-data x-init="$store.pageName = { name: 'Coupons', slug: 'sales-coupons' }">
    ...
    <div class="flex flex-wrap items-center justify-between gap-4 mb-6">
        <div class="grid grid-cols-4 gap-3 flex-1">
            ...
            <div class="bg-white rounded-xl border border-gray-200 px-4 py-3">
                <p class="text-xs text-gray-400">Total Usages</p>
                <p class="text-xl font-semibold text-indigo-600 mt-0.5">{{ $totalUsages }}</p>
            </div>
            <div class="bg-white rounded-xl border border-gray-200 px-4 py-3">
                <p class="text-xs text-gray-400">Discount Given</p>
                <p class="text-xl font-semibold text-red-500 mt-0.5">{{ number_format($totalDiscountGiven, 2) }}</p>
            </div>
        </div>
        <div class="flex items-center gap-2 shrink-0">
            <a href="{{ route('admin.sales.coupons.usages') }}" wire:navigate
            ...
            <a href="{{ route('admin.sales.coupons.create') }}" wire:navigate
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
        ...
        <div class="flex flex-wrap items-center gap-3 px-5 py-4 border-b border-gray-100">
            <div class="relative flex-1 min-w-[200px]">
                ...
                <input wire:model.live.debounce.300ms="search" type="text" placeholder="Search by code or name…"
            ...
            <table class="min-w-full">
                <thead>
                    <tr class="border-b border-gray-100 bg-gray-50/40">
                        ...
                        <th class="px-5 py-3 text-center text-xs font-semibold text-gray-500 uppercase tracking-wide">Usages</th>
                        <th class="px-5 py-3 text-right text-xs font-semibold text-gray-500 uppercase tracking-wide">Min Order</th>
                ...
                <tbody class="divide-y divide-gray-100">
                    @forelse($coupons as $coupon)
                        <tr class="hover:bg-gray-50/50 transition">
                            <td class="px-5 py-3">
                                <a href="{{ route('admin.sales.coupons.show', $coupon->id) }}" wire:navigate class="hover:underline">
                                    <span class="text-sm font-mono font-semibold text-indigo-600">{{ $coupon->code }}</span>
                                </a>
                            </td>
                            <td class="px-5 py-3">
                                <span class="text-sm text-gray-700">{{ $coupon->promotion->name }}</span>
                            </td>
                            <td class="px-5 py-3 text-center">
                                <span class="text-sm text-gray-600">{{ $coupon->usages_count }}{{ $coupon->usage_limit ? ' / ' . $coupon->usage_limit : '' }}</span>
                            </td>
                            <td class="px-5 py-3 text-right">
                                <span class="text-sm text-gray-600">{{ $coupon->min_order_amount ? number_format($coupon->min_order_amount, 2) : '—' }}</span>
                            </td>
                            <td class="px-5 py-3 text-center">
                                <span class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium {{ $coupon->promotion->status->badgeClass() }}">
                                    {{ $coupon->promotion->status->label() }}
                                </span>
                            </td>
                            <td class="px-5 py-3">
                                <div class="flex items-center justify-end gap-1">
                                    <a href="{{ route('admin.sales.coupons.show', $coupon->id) }}" wire:navigate
                                    ...
                                    <button type="button" x-data
                                        @click="Swal.fire({
                                            title: 'Delete coupon?',
                                            text: '{{ addslashes($coupon->code) }} will be removed permanently.',
                                            icon: 'warning',
                                            showCancelButton: true,
                                            confirmButtonColor: '#ef4444',
                                            confirmButtonText: 'Delete'
                                        }).then(r => { if (r.isConfirmed) $wire.deleteCoupon({{ $coupon->id }}) })"
                        ...
                        <tr>
                            <td colspan="6" class="px-5 py-16 text-center">
                                <div class="flex flex-col items-center gap-3">
                                    ...
                                    <div>
                                        <p class="text-sm font-semibold text-gray-700">No coupons found</p>
                                        <p class="text-xs text-gray-400 mt-0.5">Try adjusting filters, or create a new coupon</p>
                                    </div>
                                    <a href="{{ route('admin.sales.coupons.create') }}" wire:navigate
