<div x-data x-init="$store.pageName = { name: 'Currencies', slug: 'settings-currencies' }">
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
        ...
        <div class="flex flex-wrap items-center gap-3 px-5 py-4 border-b border-gray-100">
            <div class="flex-1">
                <p class="text-sm font-medium text-gray-700">{{ $currencies->count() }} {{ Str::plural('currency', $currencies->count()) }}</p>
                <p class="text-xs text-gray-400 mt-0.5">Available currencies for use in Site Settings &rarr; Localization</p>
            </div>
        </div>
            ...
            <table class="min-w-full">
                <thead>
                    <tr class="border-b border-gray-100 bg-gray-50/40">
                        ...
                        <th class="px-5 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wide">Symbol</th>
                        <th class="px-5 py-3 text-center text-xs font-semibold text-gray-500 uppercase tracking-wide">Decimals</th>
                ...
                <tbody class="divide-y divide-gray-100">
                    @forelse($currencies as $currency)
                        <tr class="hover:bg-gray-50/50 transition group">
                            <td class="px-5 py-3">
                                <span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-mono bg-gray-100 text-gray-600">{{ $currency->code }}</span>
                            </td>
                            <td class="px-5 py-3">
                                <span class="text-sm font-medium text-gray-800">{{ $currency->name }}</span>
                            </td>
                            <td class="px-5 py-3">
                                <span class="text-sm text-gray-600">{{ $currency->symbol }}</span>
                            </td>
                            <td class="px-5 py-3 text-center">
                                <span class="text-sm text-gray-400 font-mono tabular-nums">{{ $currency->decimal_places }}</span>
                            </td>
                            <td class="px-5 py-3 text-center">
                                @if($activeCurrency === $currency->code)
                            ...
                            <td class="px-5 py-3 text-right">
                                <button type="button" x-data
                                    @click="Swal.fire({
                                        ...
                                        text: '{{ addslashes($currency->code) }} will be removed permanently.',
                                        icon: 'warning',
                                        showCancelButton: true,
                                        confirmButtonColor: '#ef4444',
                                        confirmButtonText: 'Delete'
                                    }).then(r => { if (r.isConfirmed) $wire.deleteCurrency({{ $currency->id }}) })"
                                    class="w-8 h-8 inline-flex items-center justify-center rounded-lg text-gray-400 hover:bg-red-50 hover:text-red-500 transition
                                        {{ $activeCurrency === $currency->code ? 'opacity-30 cursor-not-allowed' : '' }}"
                                    {{ $activeCurrency === $currency->code ? 'disabled' : '' }}>
                        ...
                        <tr>
                            <td colspan="6" class="px-5 py-16 text-center">
                                <div class="flex flex-col items-center gap-3">
                                    ...
                                    <div>
                                        <p class="text-sm font-semibold text-gray-700">No currencies defined</p>
                                        <p class="text-xs text-gray-400 mt-0.5">Add a currency to make it available in Site Settings</p>
                                    </div>
    ...
    <div x-cloak x-data="{ open: @entangle('createModal') }" x-show="open" x-transition
        ...
        <div class="w-full max-w-sm bg-white rounded-2xl shadow-2xl overflow-hidden">
            ...
            <div class="flex items-center gap-3 px-6 py-4 border-b border-gray-100">
                ...
                <div class="flex-1">
                    <h2 class="text-base font-semibold text-gray-900">Add Currency</h2>
                    <p class="text-xs text-gray-400">Will appear as an option in Site Settings</p>
                </div>
            ...
            <form wire:submit.prevent="createCurrency" class="px-6 py-5 space-y-4">
                <div class="grid grid-cols-2 gap-3">
                    <div>
                        <label class="block text-xs font-medium text-gray-600 mb-1.5">Code <span class="text-red-500">*</span></label>
                        <input wire:model.live="newCode" type="text" placeholder="USD" maxlength="10"
                    ...
                    <div>
                        <label class="block text-xs font-medium text-gray-600 mb-1.5">Symbol</label>
                        <input wire:model="newSymbol" type="text" placeholder="$"
                ...
                <div>
                    ...
                    <input wire:model="newName" type="text" placeholder="e.g. US Dollar"
                        class="w-full rounded-lg border border-gray-300 px-3 py-2.5 text-sm focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500">
                    @error('newName') <p class="text-xs text-red-500 mt-1">{{ $message }}</p> @enderror
                </div>
                <div>
                    <label class="block text-xs font-medium text-gray-600 mb-1.5">Decimal Places</label>
                    <input wire:model="newDecimalPlaces" type="number" min="0" max="6"
                        class="w-full rounded-lg border border-gray-300 px-3 py-2.5 text-sm focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500">
                    @error('newDecimalPlaces') <p class="text-xs text-red-500 mt-1">{{ $message }}</p> @enderror
                </div>
