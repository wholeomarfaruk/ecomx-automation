<div x-data x-init="$store.pageName = { name: 'Cities', slug: 'settings-cities' }">
        ...
        <div class="flex flex-wrap items-center gap-3 px-5 py-4 border-b border-gray-100">
            ...
            <select wire:model.live="filterCountry"
                ...
                @foreach($countries as $country)
                    <option value="{{ $country->id }}">{{ $country->emoji_flag }} {{ $country->name }}</option>
                @endforeach
            </select>

            <select wire:model.live="filterState" wire:key="filter-state-select-{{ $filterCountry }}"
                class="w-full sm:w-auto rounded-lg border border-gray-300 px-3 py-2 text-sm bg-white text-gray-700 focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500"
                @if($filterCountry === '') disabled @endif>
            ...
            <span class="text-sm text-gray-400 sm:ml-auto whitespace-nowrap">{{ $cities->total() }} cities</span>
        </div>
            ...
            <table class="min-w-full">
                <thead>
                    <tr class="border-b border-gray-100 bg-gray-50/40">
                        <th class="px-5 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wide">City / Upazila</th>
                        <th class="px-5 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wide">State</th>
                        ...
                        <tr class="hover:bg-gray-50/50 transition group">
                            <td class="px-5 py-3">
                                <div class="flex flex-col">
                                    ...
                                    @if($city->local_name)
                                        <span class="text-xs text-gray-400">{{ $city->local_name }}</span>
                                    @endif
                                </div>
                            </td>
                            <td class="px-5 py-3 text-sm text-gray-600">{{ $city->state?->name }}</td>
                            <td class="px-5 py-3">
                                <span class="inline-flex items-center gap-1.5 text-sm text-gray-600">
                                    <span class="text-base leading-none">{{ $city->country?->emoji_flag ?? '🌐' }}</span>
                                    {{ $city->country?->name }}
                                </span>
                            </td>
                            <td class="px-5 py-3">
                            ...
                            <td class="px-5 py-3 text-center">
                                <button wire:click="toggleActive({{ $city->id }})" type="button"
                                    ...
                                    class="relative inline-flex h-5 w-9 items-center rounded-full transition-colors focus:outline-none focus:ring-2 focus:ring-offset-1
                                        {{ $city->is_active ? 'bg-indigo-500 focus:ring-indigo-400' : 'bg-gray-300 focus:ring-gray-400' }}">
                                    <span class="inline-block h-3.5 w-3.5 transform rounded-full bg-white shadow transition-transform
                                        {{ $city->is_active ? 'translate-x-4.5' : 'translate-x-0.5' }}"></span>
                                </button>
                            </td>
                            ...
                            <td class="sticky right-0 bg-white group-hover:bg-gray-50/50 px-5 py-3 text-right">
                                <div class="inline-flex items-center gap-1">
                                    <button wire:click="editCity({{ $city->id }})" type="button"
                                    ...
                                    <button type="button" x-data
                                        @click="Swal.fire({
                                            title: 'Delete city?',
                                            text: '{{ addslashes($city->name) }} will be removed.',
                                            icon: 'warning',
                                            showCancelButton: true,
                                            confirmButtonColor: '#ef4444',
                                            confirmButtonText: 'Delete'
                                        }).then(r => { if (r.isConfirmed) $wire.deleteCity({{ $city->id }}) })"
                        ...
                        <tr>
                            <td colspan="6" class="px-5 py-16 text-center">
                                <div class="flex flex-col items-center gap-3">
                                    <div class="w-14 h-14 rounded-full bg-gray-100 flex items-center justify-center text-2xl">🏙️</div>
                                    <div>
                                        <p class="text-sm font-semibold text-gray-700">No cities found</p>
                                        <p class="text-xs text-gray-400 mt-0.5">Run the CitySeeder or add one manually</p>
                                    </div>
    ...
    <div x-cloak x-data="{ open: @entangle('createModal') }" x-show="open" x-transition
        ...
        <div class="w-full max-w-md max-h-[90vh] bg-white rounded-2xl shadow-2xl overflow-hidden flex flex-col">
            ...
            <div class="flex items-center gap-3 px-6 py-4 border-b border-gray-100 shrink-0">
                ...
                <div class="flex-1">
                    <h2 class="text-base font-semibold text-gray-900">Add City</h2>
                </div>
            ...
            <form wire:submit.prevent="createCity" class="px-6 py-5 space-y-4 overflow-y-auto">
                <div class="grid grid-cols-2 gap-3">
                    <div>
                        <label class="block text-xs font-medium text-gray-600 mb-1">Country <span class="text-red-500">*</span></label>
                        <select wire:model.live="newCountryId"
                        ...
                        @error('newCountryId') <p class="text-xs text-red-500 mt-1">{{ $message }}</p> @enderror
                    </div>
                    <div>
                        <label class="block text-xs font-medium text-gray-600 mb-1">State <span class="text-red-500">*</span></label>
                        <select wire:model="newStateId" wire:key="new-state-select-{{ $newCountryId }}"
                            class="w-full rounded-lg border border-gray-300 px-3 py-2.5 text-sm bg-white focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 disabled:bg-gray-50 disabled:text-gray-400"
                            @if($newCountryId === '') disabled title="Select a country first" @endif>
                            <option value="">{{ $newCountryId === '' ? 'Select a country first' : '— Select —' }}</option>
                            @foreach($newStates as $state)
                                <option value="{{ $state->id }}">{{ $state->name }}</option>
                            @endforeach
                        </select>
                        @error('newStateId') <p class="text-xs text-red-500 mt-1">{{ $message }}</p> @enderror
                    </div>
                </div>

                <div class="grid grid-cols-2 gap-3">
                    <div>
                        <label class="block text-xs font-medium text-gray-600 mb-1">City Name <span class="text-red-500">*</span></label>
                        <input wire:model="newName" type="text" placeholder="e.g. Narayanganj Sadar"
                    ...
                    <div>
                        ...
                        <input wire:model="newLocalName" type="text" placeholder="e.g. নারায়নগঞ্জ সদর"
                            class="w-full rounded-lg border border-gray-300 px-3 py-2.5 text-sm focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500">
                    </div>
                </div>
                ...
                <div>
                    ...
                    <input wire:model="newCode" type="text" placeholder="e.g. narayanganj-sadar"
    ...
    <div x-cloak x-data="{ open: @entangle('editModal') }" x-show="open" x-transition
        ...
        <div class="w-full max-w-md max-h-[90vh] bg-white rounded-2xl shadow-2xl overflow-hidden flex flex-col">
            ...
            <div class="flex items-center gap-3 px-6 py-4 border-b border-gray-100 shrink-0">
                ...
                <div class="flex-1">
                    <h2 class="text-base font-semibold text-gray-900">Edit City</h2>
                </div>
                <button @click="open = false" type="button"
                    class="w-8 h-8 flex items-center justify-center rounded-full text-gray-400 hover:bg-gray-100 transition">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor">
            ...
            <form wire:submit.prevent="updateCity" class="px-6 py-5 space-y-4 overflow-y-auto">
                <div class="grid grid-cols-2 gap-3">
                    <div>
                        <label class="block text-xs font-medium text-gray-600 mb-1">Country <span class="text-red-500">*</span></label>
                        <select wire:model.live="editCountryId"
                            class="w-full rounded-lg border border-gray-300 px-3 py-2.5 text-sm bg-white focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500">
                            <option value="">— Select —</option>
                            @foreach($countries as $country)
                                <option value="{{ $country->id }}">{{ $country->emoji_flag }} {{ $country->name }}</option>
                        ...
                        @error('editCountryId') <p class="text-xs text-red-500 mt-1">{{ $message }}</p> @enderror
                    </div>
                    <div>
                        <label class="block text-xs font-medium text-gray-600 mb-1">State <span class="text-red-500">*</span></label>
                        <select wire:model="editStateId" wire:key="edit-state-select-{{ $editCountryId }}"
                            class="w-full rounded-lg border border-gray-300 px-3 py-2.5 text-sm bg-white focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500"
                            @if($editCountryId === '') disabled @endif>
                            <option value="">— Select —</option>
                            @foreach($editStates as $state)
                                <option value="{{ $state->id }}">{{ $state->name }}</option>
                            @endforeach
                        </select>
                        @error('editStateId') <p class="text-xs text-red-500 mt-1">{{ $message }}</p> @enderror
                    </div>
                </div>
                ...
                <div class="grid grid-cols-2 gap-3">
                    <div>
                        <label class="block text-xs font-medium text-gray-600 mb-1">City Name <span class="text-red-500">*</span></label>
                        <input wire:model="editName" type="text" placeholder="e.g. Narayanganj Sadar"
                            class="w-full rounded-lg border border-gray-300 px-3 py-2.5 text-sm focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500">
                        @error('editName') <p class="text-xs text-red-500 mt-1">{{ $message }}</p> @enderror
                    </div>
                    <div>
                        <label class="block text-xs font-medium text-gray-600 mb-1">Local Name</label>
                        <input wire:model="editLocalName" type="text" placeholder="e.g. নারায়নগঞ্জ সদর"
                            class="w-full rounded-lg border border-gray-300 px-3 py-2.5 text-sm focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500">
                    </div>
                </div>
                ...
                <div>
                    <label class="block text-xs font-medium text-gray-600 mb-1">Code</label>
                    <input wire:model="editCode" type="text" placeholder="e.g. narayanganj-sadar"
                        class="w-full rounded-lg border border-gray-300 px-3 py-2.5 text-sm font-mono focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500">
