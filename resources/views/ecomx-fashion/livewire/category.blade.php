<div class="container section" style="margin-top:16px">
    <div class="cat-hero">
        <x-ux-img :id="$category?->banner?->getUrl() ?? ($category?->image ? asset('images/category/'.$category->image) : 'photo-1483985988355-763728e1935b')" :w="1800" :alt="$category?->name ?? 'Shop'" />
        <div class="cat-hero__overlay"></div>
        <div style="position:absolute;left:clamp(20px,4vw,48px);top:50%;transform:translateY(-50%);max-width:420px">
            <nav aria-label="Breadcrumb" style="font-size:12px;color:rgba(var(--sec-rgb),.7);margin-bottom:10px"><a href="{{ route('ecomx-fashion.home') }}" style="color:rgba(var(--sec-rgb),.7)">Home</a> / <span style="color:var(--sec)">{{ $category?->name ?? 'Shop' }}</span></nav>
            <h1 style="font-size:clamp(34px,4.4vw,56px);color:var(--sec);margin-bottom:10px">{{ $category?->name ?? 'Shop' }}</h1>
            <p style="font-size:13.5px;line-height:1.6;color:rgba(var(--sec-rgb),.85)">{{ $category?->description ?: 'The autumn wardrobe, edited down. Designed to outlast the season.' }}</p>
        </div>
    </div>

    <div style="display:flex;justify-content:space-between;align-items:flex-end;gap:16px;flex-wrap:wrap;padding:20px 0 4px">
        <p class="muted" style="font-size:13px">{{ number_format($paginator->total()) }} pieces</p>
        <div style="display:flex;align-items:center;gap:10px" x-data="{ sheet:false }">
            <div class="cat-search">
                <input type="search" wire:model.live.debounce.400ms="q" placeholder="Search products" aria-label="Search products">
                <x-icon name="search" />
            </div>

            <button type="button" class="btn btn--pill filters-toggle filters-toggle--always filters-toggle--light" @click="sheet=true">
                ⚙ Filters
                @if(count($cats) || count($sizes) || count($offers) || $q !== '')
                    <span class="filters-toggle__count">{{ count($cats) + count($sizes) + count($offers) + ($q !== '' ? 1 : 0) }}</span>
                @endif
            </button>

            {{-- Filter sheet — identical on desktop and mobile --}}
            <template x-if="sheet">
                <div class="overlay" style="z-index:210" @click.self="sheet=false" @keydown.escape.window="sheet=false">
                    <div class="sheet" role="dialog" aria-modal="true" aria-label="Filters">
                        <div class="sheet__head">
                            <span class="sheet__title">Filters</span>
                            <button type="button" class="modal__close" @click="sheet=false" aria-label="Close filters">✕</button>
                        </div>

                        <div style="display:flex;flex-direction:column;gap:24px">
                            <div style="display:flex;justify-content:space-between;align-items:center">
                                <span class="muted" style="font-size:12px">{{ number_format($paginator->total()) }} results</span>
                                <button type="button" wire:click="clearAll" style="border:none;background:none;font-size:12px;color:var(--ac2)">Clear all</button>
                            </div>
                            <div style="display:flex;flex-direction:column;gap:12px">
                                <span class="kicker" style="color:rgba(var(--pri-rgb),.45)">Search</span>
                                <input type="search" wire:model.live.debounce.400ms="q" placeholder="Search products"
                                       style="width:100%;border:1px solid rgba(var(--pri-rgb),.15);background:#fff;border-radius:10px;padding:11px 14px;font-size:13px">
                            </div>
                            <div style="display:flex;flex-direction:column;gap:12px">
                                <span class="kicker" style="color:rgba(var(--pri-rgb),.45)">Category</span>
                                @foreach($allCats as $c)
                                    <button type="button" class="filter-check" wire:click="toggleCat('{{ $c }}')">
                                        <span style="display:flex;align-items:center;gap:10px"><span class="filter-box {{ in_array($c,$cats)?'is-on':'' }}">{{ in_array($c,$cats)?'✓':'' }}</span>{{ $c }}</span>
                                    </button>
                                @endforeach
                            </div>
                            <div style="display:flex;flex-direction:column;gap:12px">
                                <span class="kicker" style="color:rgba(var(--pri-rgb),.45)">Offers</span>
                                @foreach($allOffers as $key => $label)
                                    <button type="button" class="filter-check" wire:click="toggleOffer('{{ $key }}')">
                                        <span style="display:flex;align-items:center;gap:10px"><span class="filter-box {{ in_array($key,$offers)?'is-on':'' }}">{{ in_array($key,$offers)?'✓':'' }}</span>{{ $label }}</span>
                                    </button>
                                @endforeach
                            </div>
                            <div style="display:flex;flex-direction:column;gap:12px">
                                <span class="kicker" style="color:rgba(var(--pri-rgb),.45)">Size</span>
                                <div style="display:flex;flex-wrap:wrap;gap:8px">
                                    @foreach($allSizes as $z)
                                        <button type="button" class="size-btn {{ in_array($z,$sizes)?'is-on':'' }}" style="min-width:42px;padding:9px 12px" wire:click="toggleSize('{{ $z }}')">{{ $z }}</button>
                                    @endforeach
                                </div>
                            </div>
                            <div style="display:flex;flex-direction:column;gap:12px">
                                <span class="kicker" style="color:rgba(var(--pri-rgb),.45)">Price</span>
                                <div x-data="{ live: {{ $maxPrice }} }">
                                    <input type="range" min="1000" max="15000" step="500" x-model.number="live" wire:model.live.debounce.400ms="maxPrice" style="width:100%;accent-color:var(--ac)">
                                    <div style="display:flex;justify-content:space-between;font-size:12px" class="muted"><span>৳1,000</span><span style="font-weight:600;color:var(--pri)" x-text="'Up to ৳' + live.toLocaleString('en-US')"></span></div>
                                </div>
                            </div>
                        </div>

                        <div class="sheet__foot">
                            <button type="button" class="btn btn--primary btn--block" @click="sheet=false">Show {{ number_format($paginator->total()) }} results</button>
                        </div>
                    </div>
                </div>
            </template>
        </div>
    </div>

    <div class="section" style="margin-top:12px">
        @if(count($cats) || count($sizes) || count($offers) || $q !== '')
            <div style="display:flex;flex-wrap:wrap;gap:8px;margin-bottom:18px">
                @foreach($cats as $c)<button type="button" class="chip" wire:click="toggleCat('{{ $c }}')">{{ $c }} ✕</button>@endforeach
                @foreach($sizes as $z)<button type="button" class="chip" wire:click="toggleSize('{{ $z }}')">Size {{ $z }} ✕</button>@endforeach
                @foreach($offers as $o)<button type="button" class="chip" wire:click="toggleOffer('{{ $o }}')">{{ $allOffers[$o] ?? $o }} ✕</button>@endforeach
                @if($q !== '')<button type="button" class="chip" wire:click="$set('q', '')">"{{ $q }}" ✕</button>@endif
            </div>
        @endif

        <div wire:loading.remove wire:target="toggleCat,toggleSize,toggleOffer,clearAll,q,maxPrice,gotoPage,nextPage,previousPage">
            @if(count($items))
                <div class="grid-auto">
                    @foreach($items as $p)<x-product-card :product="$p" wire:key="c-{{ $p['id'] }}" />@endforeach
                </div>

                @if($paginator->hasPages())
                    <div style="display:flex;justify-content:center;gap:8px;margin-top:44px">
                        <button type="button" class="btn btn--outline btn--pill" style="padding:10px 18px"
                                wire:click="previousPage" @disabled($paginator->onFirstPage())>← Prev</button>
                        <span class="muted" style="display:flex;align-items:center;font-size:13px">Page {{ $paginator->currentPage() }} of {{ $paginator->lastPage() }}</span>
                        <button type="button" class="btn btn--outline btn--pill" style="padding:10px 18px"
                                wire:click="nextPage" @disabled(! $paginator->hasMorePages())>Next →</button>
                    </div>
                @endif
            @else
                <div style="text-align:center;padding:80px 20px">
                    <p style="font-family:'Playfair Display',serif;font-size:24px;margin-bottom:8px">Nothing matches — yet.</p>
                    <button type="button" class="btn btn--outline btn--pill" wire:click="clearAll">Clear filters</button>
                </div>
            @endif
        </div>

        <div wire:loading wire:target="toggleCat,toggleSize,toggleOffer,clearAll,q,maxPrice,gotoPage,nextPage,previousPage" class="grid-auto">
            @foreach(range(1, 8) as $skeletonIndex)
                <div class="skel" style="aspect-ratio:3/4;border-radius:var(--radius)"></div>
            @endforeach
        </div>
    </div>
</div>
